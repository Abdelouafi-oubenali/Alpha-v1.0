...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mr-[10rem]">
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <form method="POST" action="{{ route('users.update', $user->id) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <input type="tel" name="telephone" id="telephone" value="{{ old('telephone', $user->telephone) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('telephone')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="type_contrat" id="type_contrat" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <option value="CDI" {{ old('type_contrat', $user->type_contrat) == 'CDI' ? 'selected' : '' }}>CDI</option>
                        <option value="CDD" {{ old('type_contrat', $user->type_contrat) == 'CDD' ? 'selected' : '' }}>CDD</option>
                        <option value="Intérim" {{ old('type_contrat', $user->type_contrat) == 'Intérim' ? 'selected' : '' }}>Intérim</option>
                        <option value="Freelance" {{ old('type_contrat', $user->type_contrat) == 'Freelance' ? 'selected' : '' }}>Freelance</option>
                        <option value="Stage" {{ old('type_contrat', $user->type_contrat) == 'Stage' ? 'selected' : '' }}>Stage</option>
                        <option value="Alternance" {{ old('type_contrat', $user->type_contrat) == 'Alternance' ? 'selected' : '' }}>Alternance</option>
                    </select>
                    @error('type_contrat')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="col-span-2">
                    ...
                    <div class="mt-2 flex items-center">
                        <div class="h-24 w-24 overflow-hidden rounded-full bg-gray-100 flex justify-center items-center">
                            <img id="preview" src="{{ $user->photo_profile ? asset('storage/' . $user->photo_profile) : asset('images/default-avatar.png') }}" alt="Aperçu" class="h-full w-full object-cover">
                        </div>
                        <div class="ml-5">
...
<script>
    function previewImage() {
        ...
        if (file) {
            ...
            preview.src = "{{ $user->photo_profile ? asset('storage/' . $user->photo_profile) : asset('images/default-avatar.png') }}";
        }
    }
</script>
@endsection